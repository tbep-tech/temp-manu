...
All models excluded Lower Tampa Bay because of minimal seagrass change over time. Also attempted to evaluate total and change as binary. 
...
NMS results in @fig-nms and GAM results in @fig-gamres.

GAM notation:

$$
Y_i = \beta_0 + f_1\left(\text{Sal}_i\right) + f_2\left(\text{Temp}_i\right) + f_3\left(\text{Both}_i\right) + f_4\left(\text{Year}_i, \text{Sal}_i\right) + f_5\left(\text{Year}_i, \text{Temp}_i\right) + f_6\left(\text{Year}_i, \text{Both}_i\right)  + \epsilon_i
$$
...
![Predicted results from a Generalized Additive Model evaluating frequency occurrence of seagrass versus year (left), number of days in a year when temperature exceeded the threshold (top right), and number of days in a year when salinity was below the threshold (bottom right). All variables were fit as main effects, including interactions with year. The right plots show model predicted results for the threshold variables for selected year slices. 2016 to 2022 are years with recent seagrass decline. Shaded areas are 95% confidence intervals.](figs/gamres.png){#fig-gamres}
