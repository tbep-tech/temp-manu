...
"Both" model did not include a year category and the interaction of temp/sal in the first model was negligible (fig holds temp/sal at mean? for respective plots).

Seagrass decline results in @fig-tempsalmod, @fig-sggammod
...
![Predicted results from generalized linear models evaluating seagrass frequency occurrence by time periods versus (a) the number of days when salinity was below the threshold and (b) the number of days when temperature was above the threshold. Time time periods were seagrass recovery prior to 2016 and seagrass decline 2016 to present. Shaded areas are 95% confidence intervals. Points are estimates of seagrass frequency and stressor metrics by bay segment. OTB: Old Tampa Bay, HB: Hillsborough Bay, MTB: Middle Tampa Bay.](figs/tempsalmod.png){#fig-sgmod}
